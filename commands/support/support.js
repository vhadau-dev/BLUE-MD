...
blue.bot({
  ...
  handler: async (sock, msg) => {
    const imageUrl = process.env.BLUE_IMAGE; // fetch image from .env

    if (!imageUrl) {
      return await sock.sendMessage(msg.key.remoteJid, { text: '❌ BLUE_IMAGE is not set in .env' }, { quoted: msg });
    }
...
`;
    ...
    try {
      ...
      await sock.sendMessage(msg.key.remoteJid, { text: `❌ Failed to load image from BLUE_IMAGE\nError: ${err.message}` }, { quoted: msg });
    }
  }
});
